...
void AAquasy::BeginPlay()
{
	...
	SurfTimeline.SetTimelineFinishedFunc(FOnTimelineEventStatic::CreateUObject(this, &AAquasy::ActionAfterSurf));
...
void AAquasy::MoveAquasyWhenSurfing(float Value)
{
	FVector NewLocation = FMath::Lerp(LastLoc, Destination, Value);
...
void AAquasy::Surf()
{
	...
	Destination = LastLoc + FVector(550.0f * ForwardDir, 0.0f, 0.0f);
	if (SurfCurveFloat) {
		SurfTimeline.AddInterpFloat(SurfCurveFloat, FOnTimelineFloatStatic::CreateUObject(this, &AAquasy::MoveAquasyWhenSurfing));
