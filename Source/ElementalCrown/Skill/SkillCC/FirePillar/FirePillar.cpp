...
UFirePillar::UFirePillar() : USkillCC(TEXT("/Script/Paper2D.PaperSprite'/Game/Assets/Effect/Skill/FirePillar/Retro_Impact_Effect_Pack_5_A_Sprite_4.Retro_Impact_Effect_Pack_5_A_Sprite_4'"))
{
		...
		if (ABaseCharacter* OwningCharacter = SkillComponent->GetOwner<ABaseCharacter>()) {
			...
			if (OwnerMana > 0) {
				if (isAvailable && abs(OwningCharacter->GetVelocity().Z) <= 0.001f) {
					...
					isAvailable = false;
					GetWorld()->GetTimerManager().SetTimer(CountdownProgHandle, FTimerDelegate::CreateUObject(this, &UFirePillar::CooldownSkill), 0.1f, true);
